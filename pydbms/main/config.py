...
def validate_config_types() -> dict:
    ...
    mr = cfg.get("ui", {}).get("max_rows")
    if not (mr is None or isinstance(mr, int)):
       cfg.setdefault("ui", {})["max_rows"] = DEFAULT_CONFIG["ui"]["max_rows"]
       changed=True
       
    p = cfg.get("export", {}).get("path")
    if not (p is None or isinstance(p, str)):
       cfg.setdefault("export", {})["path"] = get_default_export_path()
       changed=True
